...
export function checkShowState(five: Five, showState: ShowState) {
  ...
  if (nearlyEqual(latitude, _latitude, 2) && nearlyEqual(longitude, _longitude, 2) && fov === _fov)
    return true
  return false
}
 ...
 */
export default async function correctFiveState(
  five: Five,
  showState: ShowState,
  userAction = true,
) {
  ...
  if (five.currentMode !== Five.Mode.Floorplan) {
    const [changeModeError] = await to(
      changeMode(five, [Five.Mode.Floorplan, showState, undefined, userAction]),
    )
